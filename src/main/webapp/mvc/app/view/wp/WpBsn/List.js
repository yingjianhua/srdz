Ext.define('mvc.view.wp.WpBsn.List',{
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '分类',width : 100,dataIndex : 'bean.ctg',sortable : true,renderer : mvc.Tools.beanRenderer()}
	,{text : '区域',width : 100,dataIndex : 'bean.area',sortable : true,renderer : mvc.Tools.beanRenderer()}
	,{text : '纬度',width : 100,dataIndex : 'bean.latitude',sortable : true,align : 'right'}
	,{text : '经度',width : 100,dataIndex : 'bean.longitude',sortable : true,align : 'right'}
	,{text : '地址',width : 100,dataIndex : 'bean.addr',sortable : true}
	,{text : '描述',width : 100,dataIndex : 'bean.des',sortable : true}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wp.WpBsn'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wp.WpBsn.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wp.WpBsn', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wp_WpBsn_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wp_WpBsn_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wp.WpBsn.WinSearch',{
