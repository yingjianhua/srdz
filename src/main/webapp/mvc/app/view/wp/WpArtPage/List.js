Ext.define('mvc.view.wp.WpArtPage.List',{
...
this.columns = [{text : '标题',width : 100,dataIndex : 'bean.title',sortable : true}
	...
	,{text : '更新时间',width : 140,dataIndex : 'bean.updateTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wp.WpArtPage'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wp.WpArtPage.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wp.WpArtPage', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wp_WpArtPage_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wp_WpArtPage_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onShareWin : function(selection) {
	...
	if (selection) {
		...
		Ext.Ajax.request({ 
			url : base_path+'/wp_WpArtPage_share?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
			async : false,
			success : function (response, options) {
				var result = Ext.decode(response.responseText);
				if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wp.WpArtPage.WinSearch',{
