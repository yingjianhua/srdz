Ext.define('mvc.view.wp.WpArtBanner.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WpArtBanner',
lock : true,
...
this.columns = [{text : '图文',width : 100,dataIndex : 'bean.art',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'wp',mn : 'view.wp.WpArt.List'}
	...
	,{text : '更新时间',width : 140,dataIndex : 'bean.updateTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wp.WpArtBanner'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wp.WpArtBanner.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wp.WpArtBanner', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wp_WpArtBanner_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wp_WpArtBanner_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wp.WpArtBanner.WinSearch',{
