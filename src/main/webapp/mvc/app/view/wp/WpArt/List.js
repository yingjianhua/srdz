Ext.define('mvc.view.wp.WpArt.List',{
...
this.columns = [{text : '图文秀',width : 100,dataIndex : 'bean.artShow',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'wp',mn : 'view.wp.WpArtShow.List'}
	...
	,{text : '主题',width : 100,dataIndex : 'bean.theme',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'wp',mn : 'view.wp.WpArtTheme.List'}
	...
	,{text : '更新时间',width : 140,dataIndex : 'bean.updateTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wp.WpArt'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wp.WpArt.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wp.WpArt', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wp_WpArt_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wp_WpArt_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wp.WpArt.WinSearch',{
