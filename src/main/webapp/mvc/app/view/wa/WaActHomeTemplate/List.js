Ext.define('mvc.view.wa.WaActHomeTemplate.List',{
extend : 'Ext.panel.Panel',
oldId : 'WaActHomeTemplate_list_',
loadMask : true,
multiSelect : true,
roles : '',
layout : 'border',
...
mainActs.push({
		...
		items : [{
				...
				items : [{
						...
						items : [{xtype : 'textfield',name : 'bean.name',fieldLabel : '名称'}
							,{xtype : 'textfield',name : 'bean.activityRule',fieldLabel : '活动规则'}
							]
					}]
			}]
	},{
		...
		items : [{
				xtype : Ext.create('mvc.view.wa.WaActHomeTemplate.ListMain',{
							title : '首页模板',
							...
							listeners : 							 {
				                ...
				                selectionchange: function(model, records) {
				                    if (records.length === 1){
        								...
        								this.mdLineTable.store.filter([{'id':'filter', 'property':'homeTemp','value':records[0].get('bean.pkey')}]);
    									if (this.roles.indexOf('upd') != -1)
				...
				xtype : Ext.create('mvc.view.wa.WaActHomeTemplate.ListLineWaActHomeTemplateBarnner',{
							title : '横幅配置',
...
onIns : function(){
		var win = Ext.create('mvc.view.wa.WaActHomeTemplate.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wa_WaActHomeTemplate_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wa.WaActHomeTemplate.WinSearch',{
