Ext.define('mvc.view.wa.WaActConfig.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WaActConfig',
lock : true,
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	...
	,{text : '无效区域',width : 100,dataIndex : 'bean.invalidArea',sortable : true}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wa.WaActConfig'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wa.WaActConfig.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wa.WaActConfig', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wa_WaActConfig_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wa_WaActConfig_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wa.WaActConfig.WinSearch',{
