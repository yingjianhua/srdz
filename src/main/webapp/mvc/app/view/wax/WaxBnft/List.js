Ext.define('mvc.view.wax.WaxBnft.List',{
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '首图',width : 100,dataIndex : 'bean.imgUrl',sortable : true,renderer:function(v) {return "<img src='../"+v+"'width='90px' height='70px'>"}}
	,{text : '介绍',width : 100,dataIndex : 'bean.des',sortable : true}
	,{text : '商家',width : 100,dataIndex : 'bean.bsn',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'wp',mn : 'view.wp.WpBsn.List'}
	...
	,{text : '活动状态',width : 60,dataIndex : 'bean.status',sortable : true,renderer : mvc.Tools.optRenderer('wa','Wa','OActVoteStatus')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wax.WaxBnft'); 
...
onChangeStatus : function(status){
	var WA_STATUS_INIT = 0, WA_STATUS_CLOSE = 1;
	if (this.roles.indexOf('upd') != -1)
		this.down('#'+this.oldId+'upd').setDisabled(status != WA_STATUS_INIT);
	if (this.roles.indexOf('del') != -1)
		this.down('#'+this.oldId+'del').setDisabled(status != WA_STATUS_INIT);
	if (this.roles.indexOf('doClose') != -1)
		this.down('#'+this.oldId+'doClose').setDisabled(status != WA_STATUS_INIT);
},
...
onIns : function(){
		var win = Ext.create('mvc.view.wax.WaxBnft.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wax.WaxBnft', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wax_WaxBnft_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wax_WaxBnft_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
				...
				Ext.MessageBox.confirm(msg_confirm_title, '确认关闭吗？',
					function(btn) {
						if (btn != 'yes')
							return;
						Ext.Ajax.request({
							url : base_path+'/wax_WaxBnft_close?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
							success : function (response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success){
									var bean  = Ext.create('mvc.model.wa.WaBnft',result);
									Ext.apply(selection.data, bean.data);
									selection.commit();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wax.WaxBnft.WinSearch',{
