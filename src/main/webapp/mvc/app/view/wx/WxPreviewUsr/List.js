Ext.define('mvc.view.wx.WxPreviewUsr.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WxPreviewUsr',
lock : true,
...
initComponent : function(){
	...
	this.columns = [
			...
			if (mainActs.length > 0)
				this.tbar=mainActs;
			this.store=Ext.create('mvc.store.wx.WxPreviewUsr'); 
...
onIns : function(){
	...
	function onTrigger(data, params) {
		Ext.Ajax.request({
			url : base_path+'/wx_WxPreviewUsr_ins?bean.pkey='+data.split(bean_split)[0],
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wx_WxPreviewUsr_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
						success : function (response, options) {
							var result = Ext.decode(response.responseText);
							if (result.success){
