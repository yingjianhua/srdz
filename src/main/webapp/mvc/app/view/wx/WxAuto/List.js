Ext.define('mvc.view.wx.WxAuto.List',{
...
this.columns = [{text : '关键字',width : 100,dataIndex : 'bean.keyword',sortable : true}
	,{text : '消息类型',width : 100,dataIndex : 'bean.msgType',sortable : true,renderer : mvc.Tools.optRenderer('wx','Wx','OMsgType')}
	,{text : '消息模板',width : 80,dataIndex : 'bean.template',sortable : true,renderer : mvc.Tools.beanRenderer()}
	,{text : '启用标志',width : 75,dataIndex : 'bean.enabled',sortable : true,renderer : mvc.Tools.optRenderer('sys','Sys','OEnabled')}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wx.WxAuto'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wx.WxAuto.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wx.WxAuto', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wx_WxAuto_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wx_WxAuto_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要启用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/wx_WxAuto_doEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
		...
		if (selection) {
			...
			Ext.MessageBox.confirm(msg_confirm_title, '您确认要停用所选的记录吗?',
					function(btn) {
						...
						Ext.Ajax.request({
							url : base_path + '/wx_WxAuto_unEnabled?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
							success : function(response, options) {
								var result = Ext.decode(response.responseText);
								if (result.success) {
									me.getStore().reload();
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wx.WxAuto.WinSearch',{
