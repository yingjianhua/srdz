Ext.define('mvc.view.wm.WmVoice.List',{
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '语音',width : 100,dataIndex : 'bean.voiceUrl',sortable : true, renderer : function(value) {return "<a href='../"+ value + "' target=_blank>点击播放</a>"}}
	,{text : '同步状态',width : 60,dataIndex : 'bean.status',sortable : true, renderer : mvc.Tools.optRenderer('wx','Wx','OSyncStatus')}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wm.WmVoice');
...
onIns : function(){
		var win = Ext.create('mvc.view.wm.WmVoice.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wm.WmVoice', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wm_WmVoice_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmVoice_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSync : function(){
	var me = this;
	var selection = this.getView().getSelectionModel().getSelection()[0];
	Ext.MessageBox.confirm(msg_confirm_title, "确认要将该语音同步到微信平台上吗？",
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmVoice_sync?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onUnsync : function(){
	var me = this;
	var selection = this.getView().getSelectionModel().getSelection()[0];
	Ext.MessageBox.confirm(msg_confirm_title, "取消同步后，语音将转为离线状态",
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmVoice_unsync?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							me.onUpdateRecord(me, result);
...
onPreview : function() {
	...
	if(selection) {
		function onTrigger(data, params) {
			Ext.Ajax.request({
				url : base_path+'/wx_WxMassMessage_preview?tempPkey='+selection.get("bean.pkey")+'&tempClass=irille.wx.wm.WmVoice&toUser='+data,
				success : function (response, options) {
					var result = Ext.decode(response.responseText);
					if (result.success){
						Ext.example.msg(msg_title, msg_text);
...
onMass : function(){
	...
	var win = Ext.create("mvc.view.wx.WxMassMessage.WinMassMessage", {
		...
		tempClass : "irille.wx.wm.WmVoice"
	});
	win.show();
},
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wm.WmVoice.WinSearch',{
