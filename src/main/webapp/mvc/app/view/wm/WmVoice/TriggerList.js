Ext.define('mvc.view.wm.WmVoice.TriggerList',{
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '语音',width : 100,dataIndex : 'bean.voiceUrl',sortable : true, renderer : function(value) {return "<a href='" + base_path +"/"+ value + "' target=_blank>点击播放</a>"}}
	,{text : '同步状态',width : 60,dataIndex : 'bean.status',sortable : true,renderer : mvc.Tools.optRenderer('wx','Wx','OSyncStatus')}
	];
		this.store=Ext.create('mvc.store.wm.WmVoice');this.dockedItems = [{
		dock : 'top',
		xtype : 'toolbar',
		items : ["搜索：",{
				xtype : 'combo',
