Ext.define('mvc.view.wm.WmText.List',{
...
this.columns = [{text : '模板名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '模板内容',width : 100,dataIndex : 'bean.content',sortable : true}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wm.WmText'); 
...
listeners : {
	selectionchange : function(selModel, selected){
		...
		if (this.roles.indexOf('mass') != -1)
			this.down('#'+this.oldId+'mass').setDisabled(selected.length === 0);	
		if (this.roles.indexOf('preview') != -1)
			this.down('#'+this.oldId+'preview').setDisabled(selected.length === 0);	
}
},
...
onIns : function(){
		var win = Ext.create('mvc.view.wm.WmText.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wm.WmText', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wm_WmText_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmText_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onPreview : function(){
	...
	if(selection) {
		function onTrigger(data, params) {
			Ext.Ajax.request({
				url : base_path+'/wx_WxMassMessage_preview?tempPkey='+selection.get("bean.pkey")+'&tempClass=irille.wx.wm.WmText&toUser='+data,
				success : function (response, options) {
					var result = Ext.decode(response.responseText);
					if (result.success){
						Ext.example.msg(msg_title, msg_text);
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wm.WmText.WinSearch',{
