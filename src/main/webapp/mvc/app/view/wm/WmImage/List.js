Ext.define('mvc.view.wm.WmImage.List',{
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '图片',width : 300,dataIndex : 'bean.imgUrl',sortable : true, renderer: function(v) {return "<img src='../"+v+"' height='100px'>"}}
	,{text : '同步状态',width : 60,dataIndex : 'bean.status',sortable : true,renderer : mvc.Tools.optRenderer('wx','Wx','OSyncStatus')}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wm.WmImage'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wm.WmImage.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wm.WmImage', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wm_WmImage_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmImage_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSync : function(){
	var me = this;
	var selection = this.getView().getSelectionModel().getSelection()[0];
	Ext.MessageBox.confirm(msg_confirm_title, "确认要将该图片同步到微信平台上吗？",
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmImage_sync?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onUnsync : function(){
	var me = this;
	var selection = this.getView().getSelectionModel().getSelection()[0];
	Ext.MessageBox.confirm(msg_confirm_title, "取消同步后，图片将转为离线状态",
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wm_WmImage_unsync?pkey='+selection.get('bean.pkey')+'&rowVersion='+selection.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
							me.onUpdateRecord(me, result);
...
onPreview : function() {
	...
	if(selection) {
		function onTrigger(data, params) {
			Ext.Ajax.request({
				url : base_path+'/wx_WxMassMessage_preview?tempPkey='+selection.get("bean.pkey")+'&tempClass=irille.wx.wm.WmImage&toUser='+data,
				success : function (response, options) {
					var result = Ext.decode(response.responseText);
					if (result.success){
						Ext.example.msg(msg_title, msg_text);
...
onMass : function(){
	...
	var win = Ext.create("mvc.view.wx.WxMassMessage.WinMassMessage", {
		...
		tempClass : "irille.wx.wm.WmImage"
	});
	win.show();
},
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wm.WmImage.WinSearch',{
