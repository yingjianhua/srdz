Ext.define('mvc.view.wm.WmVideo.TriggerList',{
...
this.columns = [{text : '标题',width : 100,dataIndex : 'bean.title',sortable : true}
	...
	,{text : '视频',width : 100,dataIndex : 'bean.videoUrl',sortable : true, renderer : function(value) {return "<a href='" + base_path +"/"+ value + "' target=_blank>点击播放</a>"}}
	,{text : '同步状态',width : 60,dataIndex : 'bean.status',sortable : true,renderer : mvc.Tools.optRenderer('wx','Wx','OSyncStatus')}
	];
		this.store=Ext.create('mvc.store.wm.WmVideo');this.dockedItems = [{
		dock : 'top',
		xtype : 'toolbar',
		items : ["搜索：",{
				xtype : 'combo',
