Ext.define('mvc.view.wx.WxUser.TriggerList',{
...
this.columns = [{text : '关注状态',width : 60,dataIndex : 'bean.status',sortable : true,renderer : mvc.Tools.optRenderer('wx','Wx','OStatus')}
	...
	,{text : '唯一ID',width : 100,dataIndex : 'bean.unionId',sortable : true}
		...
		this.store=Ext.create('mvc.store.wx.WxUser');this.dockedItems = [{
		dock : 'top',
		xtype : 'toolbar',
		items : ["搜索：",{
				xtype : 'combo',
...
onTriggerList : function(){
		...
		if (selection){
			this.fireEvent('trigger', selection.get('bean.pkey') + bean_split + selection.get('bean.nickname'), null);
		}
}
});