Ext.define('mvc.view.wpt.WptMenu.Win',{
...
initComponent : function(){
	...
	anchor : '100%',
	plain : true,
	xtype : Ext.create('mvc.view.wpt.WptMenu.Form',{	insFlag : this.insFlag})
},{
	xtype : Ext.create('mvc.view.wpt.WptMenu.ListForm',{height : 300,border : false })
}];
...
onSave : function(){
		...
		if (form.isValid()) {
			form.submit({
				url : base_path+'/wpt_WptRestaurant_insOrUpd',
	...
	store.each(function(record) {
		for (f = 0; f < fLen; f++) {
			...
			if (value || value===0){
        		...
        		values['menuList'+'['+i+'].'+fields[f].mapping] = value;
			}
    	}
    	i++;
	});
