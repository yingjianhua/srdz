Ext.define('mvc.view.wpt.WptRestaurantBsn.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WptRestaurantBsn',
lock : true,
...
this.columns = [{text : '餐厅',width : 100,dataIndex : 'bean.restaurant',sortable : true,renderer : mvc.Tools.beanRendererHref(),md : 'wpt',mn : 'view.wpt.WptRestaurant.List'}
	...
	,{text : '创建时间',width : 140,dataIndex : 'bean.createTime',sortable : true,renderer : Ext.util.Format.dateRenderer('Y-m-d H:i:s')}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wpt.WptRestaurantBsn'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wpt.WptRestaurantBsn.Win',{
			title : this.title+'>新增'
		});
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wpt_WptRestaurantBsn_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wpt.WptRestaurantBsn.WinSearch',{
