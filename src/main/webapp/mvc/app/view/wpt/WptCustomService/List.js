Ext.define('mvc.view.wpt.WptCustomService.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WptCustomService',
lock : true,
...
this.columns = [{text : '名称',width : 100,dataIndex : 'name',sortable : true}
	...
	,{text : '金额',width : 100,dataIndex : 'price',sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wpt.WptCustomService'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wpt.WptCustomService.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wpt.WptCustomService', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wpt_WptCustomService_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wpt_WptCustomService_del?pkey='+row.get('pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wpt.WptCustomService.WinSearch',{
