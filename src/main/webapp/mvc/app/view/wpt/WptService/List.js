Ext.define('mvc.view.wpt.WptService.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WptService',
lock : true,
...
this.columns = [{text : '名称',width : 100,dataIndex : 'bean.name',sortable : true}
	,{text : '图标',width : 100,dataIndex : 'bean.icon',sortable : true,renderer:function(v) {return "<img src='../"+v+"'width='90px' height='70px'>"}}
	,{text : '金额',width : 100,dataIndex : 'bean.price',sortable : true,renderer : mvc.Tools.numberRenderer(),align : 'right'}
	];
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wpt.WptService'); 
...
onIns : function(){
		var win = Ext.create('mvc.view.wpt.WptService.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wpt.WptService', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wpt_WptService_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
		...
		Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg,
			function(btn) {
				if (btn != 'yes')
					return;
				Ext.Ajax.request({
					url : base_path+'/wpt_WptService_del?pkey='+row.get('bean.pkey')+'&rowVersion='+row.get(BEAN_VERSION),
					success : function (response, options) {
						var result = Ext.decode(response.responseText);
						if (result.success){
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wpt.WptService.WinSearch',{
