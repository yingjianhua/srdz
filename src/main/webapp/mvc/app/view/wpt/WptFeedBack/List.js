Ext.define('mvc.view.wpt.WptFeedBack.List',{
extend : 'Ext.grid.Panel',
oldId : 'btn_WptFeedBack',
lock : true,
...
var mainActs = [];		
	if (this.roles.indexOf('toDo') != -1)
mainActs.push({
		text : '处理',
		iconCls : 'ins-icon',
		itemId : this.oldId+'toDo',
		scope : this,
		handler : this.onToDo,
		disabled : this.lock
	});
this.columns = [{text : '内容',width : 100,dataIndex : 'content',sortable : true}
	...
	,{text : '已处理',width : 100,dataIndex : 'isHandle',sortable : true,renderer : function(v) {return v?"是":"否"}}
		...
		if (mainActs.length > 0)
			this.tbar=mainActs;
		this.store=Ext.create('mvc.store.wpt.WptFeedBack'); 
...
listeners : {
	selectionchange : function(selModel, selected){
		if (this.roles.indexOf('toDo') != -1)
			this.down('#'+this.oldId+'toDo').setDisabled(selected.length === 0);	
}
},
...
onIns : function(){
		var win = Ext.create('mvc.view.wpt.WptFeedBack.Win',{
			title : this.title+'>新增'
		});
...
onUpdateRecord : function(form, data){
		...
		var bean = Ext.create('mvc.model.wpt.WptFeedBack', data);
		...
		if (selection){
			...
			Ext.MessageBox.confirm(msg_confirm_title, msg_confirm_msg, 
				function(btn) {
					...
					Ext.Ajax.request({
						url : base_path+'/wpt_WptFeedBack_delMulti?pkeys='+arr.toString()+'&rowVersions='+arrv.toString(),
...
onSearchAdv : function(){
		var win = Ext.create('mvc.view.wpt.WptFeedBack.WinSearch',{
...
onToDo : function(){
	...
	if (selection) {
		var me = this;
		Ext.MessageBox.confirm(msg_confirm_title, '您确认要处理吗?',
				function(btn) {
					if (btn != 'yes')
						return;
					var arr = new Array();
