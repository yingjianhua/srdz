Ext.define('mvc.view.wpt.WptServiceCenter.List', {
	extend : 'Ext.grid.Panel',
	oldId : 'btn_WptServiceCenter',
	lock : true,
	...
	initComponent : function() {
		...
		if (mainActs.length > 0)
			this.tbar = mainActs;
		this.store = Ext.create('mvc.store.wpt.WptServiceCenter');
	...
	onWxTip : function() {
		...
		var win = Ext.create("Ext.window.Window", {
		    ...
		    items: Ext.create("mvc.view.wpt.WptWxTips.List",{
		    	account:me.store.getAt(0).get('pkey') 
		    })
		}).show();
		win.down("grid").store.filter([{'id':'filter', 'property':'account','value':this.store.getAt(0).get('pkey')}]);
	},
	onSearch : function() {
	...
	onSetWin : function(selection) {
		var win = Ext.create('mvc.view.wpt.WptServiceCenter.Win', {
			title : this.title + '>设置',
		});
		win.show();
