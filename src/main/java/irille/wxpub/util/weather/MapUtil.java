...
public class MapUtil {
	...
	private static void iterateWholeXML(String filename, HashMap<String, String> map) {
		...
		try {
			...
			for (Iterator<?> iterProvince = root.elementIterator(); iterProvince.hasNext();) {
				Element province = (Element) iterProvince.next();
				for (Iterator<?> iterCity = province.elementIterator(); iterCity.hasNext();) {
					Element city = (Element) iterCity.next();
					for (Iterator<?> iterCountry = city.elementIterator(); iterCountry.hasNext();) {
